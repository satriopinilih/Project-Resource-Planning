...
export default function HRDashboard() {
  ...
  const [approveDeclineError, setApproveDeclineError] = useState<string | null>(null);
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      setApproveDeclineError(e instanceof Error ? e.message : 'Failed to approve request');
    } finally {
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      setApproveDeclineError(e instanceof Error ? e.message : 'Failed to decline request');
    } finally {
      setIsProcessing(false);
    }
  };
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen">
      ...
      <Modal isOpen={isApprovalModalOpen} onClose={() => { setIsApprovalModalOpen(false); setApproveDeclineError(null); }} title="Approve Contract Extension">
        <div className="space-y-4">
          ...
          <div className="flex gap-3 justify-end">
            <button onClick={() => { setIsApprovalModalOpen(false); setApproveDeclineError(null); setReviewNotes(''); }} className="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg">Cancel</button>
            <button onClick={handleApprove} disabled={!reviewNotes.trim() || isProcessing} className="px-4 py-2 bg-green-600 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
      ...
      <Modal isOpen={isDeclineModalOpen} onClose={() => { setIsDeclineModalOpen(false); setApproveDeclineError(null); }} title="Decline Contract Extension">
        <div className="space-y-4">
          <div className="space-y-1">
            <p className="text-xs text-gray-500 dark:text-gray-400">Employee</p>
            <p className="text-sm font-semibold text-gray-900 dark:text-white">{selectedRequest?.employeeName ?? '-'}</p>
          ...
          <div className="flex gap-3 justify-end">
            <button onClick={() => { setIsDeclineModalOpen(false); setApproveDeclineError(null); setReviewNotes(''); }} className="px-4 py-2 border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-lg">Cancel</button>
            <button onClick={handleDecline} disabled={!reviewNotes.trim() || isProcessing} className="px-4 py-2 bg-red-600 text-white rounded-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
              {isProcessing && <span className="w-3.5 h-3.5 border-2 border-white/40 border-t-white rounded-full animate-spin" />}
