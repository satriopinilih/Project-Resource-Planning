...
function PMProjectsContent() {
    ...
    const fetchProjects = async () => {
      try {
        ...
        const mappedData: Project[] = activeProjects.map((p) => {
          ...
          // The PM who is currently active (status = Assigned)
          const activePmMember = p.members?.find(
            (m) =>
              (m.role?.toLowerCase() === "pm" ||
                m.role?.toLowerCase().includes("manager")) &&
              m.status === "Assigned",
          ...
          // Always start with the real DB status
          let displayStatus = mapStatus(p.projectStatus);

          // Only override to "Completed (Replaced)" for the PM who was swapped out
          if (swappedPmMember && swappedPmMember.userId === currentPmId) {
            displayStatus = "Completed (Replaced)";
          }

          // Show the current user's own ID if they were swapped out, otherwise show the active PM
          const displayPm =
            (swappedPmMember?.userId === currentPmId
              ? swappedPmMember?.userId
              : activePmMember?.userId) ||
            p.members?.find(
              (m) =>
                m.role?.toLowerCase() === "pm" ||
                m.role?.toLowerCase().includes("manager"),
  ...
  const filteredProjects = projectsData.filter((project) => {
    const matchesTab =
      ...
      (activeTab === "Completed" && project.status === "Completed (Replaced)");
    const matchesSearch =
          ...
          {showFilters && (
            <div className="mx-5 mt-4 rounded-xl border border-[var(--dash-border)] bg-[var(--dash-bg-elevated)] p-4">
              <div className="flex items-center justify-between mb-3">
                <h4 className="text-[13px] font-semibold text-[var(--dash-text-heading)]">
                  Filters &amp; Sorting
                </h4>
                <button
                  onClick={() => {
              ...
              <tbody>
                {isLoading ? (
                  <tr>
                    <td
                      colSpan={6}
                      className="py-8 text-center text-[var(--dash-text-muted)]"
                    >
                  ...
                  filteredProjects.map((project) => (
                    ...
                    >
                      ...
                      <td className="py-4 pr-6 pl-4 text-right">
                        <ArrowRight
                          size={14}
                          className="text-gray-600 opacity-0 group-hover:opacity-100 translate-x-[-10px] group-hover:translate-x-0 transition-all"
                        />
                      </td>
                    </tr>
                  ))
